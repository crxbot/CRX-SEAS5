...
OM_FILENAME_PATTERNS = {
    ...
    "t2m_max_spread": "temperature_2m_max_spread",
    "t2m_min": "temperature_2m_min",
    "t2m_min_spread": "temperature_2m_min_spread",
}
...
# ------------------------------
EMBED_DATA_VARS = {"t2m_max", "t2m_max_spread", "t2m_min", "t2m_min_spread"}
...
# ------------------------------
MEAN_TO_SPREAD_VAR = {
    "t2m_max": "t2m_max_spread",
    "t2m_min": "t2m_min_spread",
}
...
COLORMAPS = {
    "t2m_max": (t2m_colors, t2m_norm),
    "t2m_min": (t2m_colors, t2m_norm),
}

# Umrechnung Rohwert -> Anzeige-Einheit, je Variable
UNIT_CONVERT = {
    "t2m_max": lambda v: v - 273.15 if np.nanmax(v) > 200 else v,  # K -> °C, nur falls noetig
    "t2m_max_spread": lambda v: v,
    "t2m_min": lambda v: v - 273.15 if np.nanmax(v) > 200 else v,  # K -> °C, nur falls noetig
    "t2m_min_spread": lambda v: v,
}
...
QUANTUM_STEP = {
    "t2m_max": 0.05,   # °C
    "t2m_max_spread": 0.05,  # K
    "t2m_min": 0.05,   # °C
    "t2m_min_spread": 0.05,  # K
}
...
for filename in matching_files:
    ...
    with OmFileReader(om_path) as root:
        ...
        if var_type in [ "t2m_max_spread","t2m_min_spread"]:
        ...
        for t_idx in range(ntime):
            ...
            # ------------------------------
            outname = f"{var_type}_{valid_times_utc[t_idx].astimezone(ZoneInfo('Europe/Berlin')):%Y%m%d}.webp"
